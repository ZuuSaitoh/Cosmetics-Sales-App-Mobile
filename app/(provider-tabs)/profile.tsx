...
export default function ProviderProfileScreen() {
  ...
  // --- HÀM GỌI API CẬP NHẬT PROFILE SHOP ---
  const handleUpdateProfile = async () => {
    ...
    setIsSaving(true);
    try {
      // Body request khớp với Swagger
      const payload = {
        shopName: editShopName,
        bio: editBio,
        bankName: editBankName,
        bankAccountNumber: editBankAccountNumber,
        shopAddressId: profile?.shopAddressId || 0 // Giữ nguyên addressId cũ để tránh lỗi null
      };
      ...
      if (response.data.code === 0) {
        ...
        setProfile(response.data.result); // Cập nhật lại giao diện
