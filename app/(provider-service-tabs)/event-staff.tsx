...
export default function EventStaffScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.data.code === 0) {
        const staffs = (res.data.result || []).filter(
          (s: any) => s.serviceType === "EVENT_STAFF",
        );
        setServices(staffs);
      }
    } catch (err) {
      console.error("Lỗi lấy dữ liệu staff:", err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Staff sự kiện</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Chưa có staff nào.</Text>
          }
        />
      )}
    </SafeAreaView>
